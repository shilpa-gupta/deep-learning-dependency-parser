...
class ParsingSystem:
    ...
    def canApply(self, c, t):
        ...
        """
        This method is responsible for applying the different operations in current configuration
        including shift, left Arc and right Arc as per the paper
        "A Fast and Accurate Dependecy Parser using Neural Networks"
        """
    def apply (self, c, t):
        if t[0] == 'L':
            wi = c.getStack(1)
            ...
            c.addArc(wj, wi, t[2:-1])
        elif t[0] == 'R':
            wi = c.getStack(1)
            wj = c.getStack(0)
            c.removeTopStack()
            c.addArc(wi, wj, t[2:-1])
        else:
            c.shift()
        return c
